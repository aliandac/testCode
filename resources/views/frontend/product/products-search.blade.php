...
<div class="container" style="    padding-top: 40px;">
    <!--<ul class="exo-menu">
        @foreach($firstCategory as $first)
            <li class="mega-drop-down"><a href="#"> {{ $first->title }}</a>
                <div class="animated fadeIn mega-menu">
                    <div class="mega-menu-wrap">
                        <div class="row">
                            @foreach($first->getDetail as $detail)
                                <div class="col-md-3 col-sm-6">
                                    <h4 class="row mega-title">{{ $detail->getCategoryDetail->name }}</h4>
                                    <ul class="stander">
                                        @foreach($detail->getCategoryDetail->children->take(6) as $children)
                                            <li>
                                                <a href="{{route('productCategory',['slug'=>$children->seo_url,'id'=>$children->id])}}">{{ $children->name }}</a>
                                            </li>
                                        @endforeach
                                        <li><a class="view-more btn- btn-sm"
                                               href="{{ route('productFirstCategory',['slug'=>$detail->getCategoryDetail->seo_url,'id'=>$detail->getCategoryDetail->id]) }}">Tümünü
                                                Gör</a></li>
                                    </ul>
                                </div>
                            @endforeach
...
<div class="container page-breadcrumb">
    <div class="row"><a href="{{route('home')}}" title="Anasayfa">Anasayfa</a> / <a href="{{route('products')}}" title="Ürünler"> Ürünler </a>/ {{ $query }}</div>
</div>
