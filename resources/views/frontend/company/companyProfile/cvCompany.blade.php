        ...
        <div class="row">
            ...
            <div class="col-md-9 col-md-push-3">
                <div class="page-content company-profile">
                    ...
                    @if($companyCv->count() > 0)
                    <div class="col-md-12">
                            <div class="col-md-12"><span style="    font-size: 18px; font-weight: bold; "> Açık Pozisyonlar </span>
                            <hr>
                            </div>
                        <div class="row">
                        @foreach( $companyCv as $value)
                            <a href="{{ route('workingPersonDetail',['slug'=> Str::slug($value->title) ,'id'=>$value->id]) }}" title="{{$value->title}}">
                            <div class="col-md-12 col-sm-4 col-xs-6" style="background-color: #f3f3f3; margin-top: 10px; padding: 15px;    border: 2px solid #fcc800;">
                                <div class="row">
                                    <div class="col-md-12">
                                     <span>   {{$value->title}}</span>
                                    </div>
                                    <div class="col-md-12">
                                     <small>  {{ $value->getCompany->name }}</small>
                                    </div>
                                    <div class="col-md-6">
                                       <span> {{ $value->getCountry->name }} / {{ $value->getCity->name }}</span>
                                    </div>
                                    <div class="col-md-6">
                                       <span style="float: right"> {{ $value->created_at }}</span>
                                    </div>
                                </div>
                            </div>
                            </a>
                    ...
                    @else
                        <div class="col-md-12" style="text-align: center; padding-top: 40px;">
                            <span style="font-size: 25px;  font-weight: bold;">Firmanın İş Fırsatlarını Görmek İstediğinizi Firmaya Bildirin </span>
